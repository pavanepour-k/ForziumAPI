...
if not hasattr(server, 'add_route'):
    def mock_add_route(_method: str, _path: str, _handler: Any) -> None:
        ...
        Args:
            _method: HTTP method (unused in mock)
            _path: URL path (unused in mock)
            _handler: Request handler (unused in mock)
        """
        # Mock implementation - does nothing
        return
    server.add_route = mock_add_route  # type: ignore[assignment,attr-defined]
...
def secure_data(
    ...
    Note: api_key parameter is intentionally unused as it's used for
    dependency injection validation via api_key_query.
    """
